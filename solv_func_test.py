...
def agent_solve(nlinc, args):
    ...
    #
    dec_vec = utility + (util_par['beta'] * np.tile(args['V_nex'], args['points']))

    dec_mat = np.reshape(dec_vec, (args['points'], args['points']))

    a_p_loc = np.argmax(dec_mat, 1) + np.arange(args['points']) * 1000

    #
    # # Tells us the location in the vector of the optimal choice of a_p

    a_p = np.tile(args['a_grid'], args['points'])[a_p_loc]
    V_n = dec_vec[a_p_loc]
    c = c[a_p_loc]
    l = l[a_p_loc]
...
def para_type_steady(y):
    ...
    for j in range(life - 1, - 1, -1):
        ...
        nlinc = ((1.0 + (y['r'] * (1 - y['taxes']['capital tax'][j]))) * np.repeat(a_grid, a_grid.shape[0])) - np.tile(a_grid, a_grid.shape[0])

        a_p, V_n, c, l = (agent_solve(nlinc, y))
    ...
    while death_dist > death_tol:
        ...
        bequest_new = (1 - bq[-1]) * (1 + y['r']) * a_path[-1]
